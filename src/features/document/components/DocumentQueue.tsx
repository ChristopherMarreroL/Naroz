      ...
      {items.map((item, index) => (
        <article key={item.id} className="panel-subtle flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" className="btn-secondary" onClick={() => onMoveUp(index)} disabled={disabled || index === 0}>Arriba</button>
            <button type="button" className="btn-secondary" onClick={() => onMoveDown(index)} disabled={disabled || index === items.length - 1}>Abajo</button>
            <button type="button" className="rounded-full border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-semibold text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60" onClick={() => onRemove(item.id)} disabled={disabled}>Quitar</button>
          </div>
        </article>
      ))}
    </div>
