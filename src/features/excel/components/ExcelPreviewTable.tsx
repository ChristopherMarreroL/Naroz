          ...
          <thead className="sticky top-0 z-10 bg-slate-950 text-slate-50">
            <tr>
              {sheet.headers.map((header, columnIndex) => {
                ...
                return (
                  <th key={columnId} className="min-w-48 border-r border-white/10 px-3 py-3 align-top last:border-r-0">
            ...
            {previewRows.map((row, rowIndex) => (
              <tr key={`${file.id}-${sheet.name}-row-${rowIndex}`} className="odd:bg-white even:bg-slate-50">
                {sheet.headers.map((_, columnIndex) => (
                  <td key={`${file.id}-${sheet.name}-${rowIndex}-${columnIndex}`} className="min-w-48 max-w-72 border-r border-b border-slate-100 px-3 py-2 text-slate-700 last:border-r-0">
